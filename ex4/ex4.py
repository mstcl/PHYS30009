...
def run_option_b(global_vars: dict):
    ...
    """
    # NOTE: Nice figure 8 parameters (0, -1, -10591, 0)
    x_0 = global_vars["radius_earth"] * float(
        take_input_general("initial x displacement in multiples of R_E (default: 0)")
        or "0"
    )
    ...
    animate_orbit_1body(
        global_vars,
        np.concatenate((x_pos[:200:5], x_pos[200:stop:skip]), axis=None)
        / global_vars["radius_earth"],
        np.concatenate((y_pos[:200:5], y_pos[200:stop:skip]), axis=None)
        / global_vars["radius_earth"],
        "",
        True,
    )
...
def run_option_c(global_vars: dict):
    ...
    #     plt.ylim([-80, 80])
    #     plt.scatter(
    #         x_pos_earth[idx] / global_vars["radius_earth"],
    #         y_pos_earth[idx] / global_vars["radius_earth"],
    #         color="blue",
    #     )
    #     plt.scatter(
    #         x_pos_moon[idx] / global_vars["radius_earth"],
    #         y_pos_moon[idx] / global_vars["radius_earth"],
    #         color="red",
    #     )
    #     plt.show()
...
def get_initial_ks_2body(
    global_vars: dict, variables_1: np.ndarray, variables_2: np.ndarray, is_moon: bool
) -> tuple:
    ...
    return (
        ...
        -global_vars["grav_constant"]
        * (
            global_vars["mass_earth"] * is_moon
            + global_vars["mass_moon"] * (not is_moon)
        )
        * (x - x_prime)
        / denom,
        -global_vars["grav_constant"]
...
def get_all_ks(
    ...
    if bodies == 1:
        ...
        k1 = np.array(
            get_initial_ks_2body(global_vars, variables_1, variables_2, is_moon)
        )
        k2 = np.array(
            get_initial_ks_2body(global_vars, variables_1, variables_2, is_moon)
        )
...
def populate_rk4_2body(
    ...
    for idx in range(np.size(t)):
        ...
        k1, k2, k3, k4 = get_all_ks(global_vars, variables_1, variables_2, False, 2)
        ...
        k1, k2, k3, k4 = get_all_ks(global_vars, variables_2, variables_1, True, 2)
...
def populate_rk4_1body(
    ...
    for idx in range(np.size(t)):
        ...
        k1, k2, k3, k4 = get_all_ks(global_vars, variables, np.array([]), is_moon, 1)
...
def animate_orbit_2body(
    ...
    # xmax = np.max([np.abs(np.min(x_pos_earth)), np.max(x_pos_earth)])
    # ymax = np.max([np.abs(np.min(y_pos_earth)), np.max(y_pos_earth)])
    ...
    def animate(i):
        ...
        ax.plot(x_pos_earth[i], y_pos_earth[i], color="blue", marker="o", ms=3)  # type: ignore
        ax.plot(x_pos_moon[i], y_pos_moon[i], color="olive", marker="o", ms=2)  # type: ignore
...
def animate_orbit_1body(
    ...
    def animate(i):
        ...
        if is_moon:
            ...
            ax.set_ylim([-1.1, ani_scale * 1.1])  # type: ignore
...
def plot_overview_orbit(x_pos: np.ndarray, y_pos: np.ndarray):
    ...
    plt.xlabel(r"x distance ($R_E$)")
    plt.ylabel(r"y distance ($R_E$)")
    plt.xlim([-10, 10])
    plt.ylim([-1.4, 65])
    plt.plot(
        x_pos,
    ...
    moon_coord = [-0.274, 0.274]
    plt.plot(moon_coord, np.ones_like(moon_coord) * 60.18, ms=6)
    plt.show()
