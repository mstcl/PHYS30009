...
def calculate_next_timestep(position: float, time: float, global_vars: dict, wavepacket: StringWave):
    """TODO: Docstring for calculate_next_timestep.
    :returns: TODO

    """
    u_x_plus_h = evaluate_displacement((position + global_vars["timestep2"]))


def evaluate_displacement(position: float, time: float, wavepacket: StringWave):
    ...
    return (
        0
        if position in [0, wavepacket.length]
        else wavepacket.get_a_0()
        * np.exp(
            -(position - wavepacket.get_phase_velocity() * time) / (wavepacket.std) ** 2
        )
    )


def evaluate_time_derivative(position: float, time: float, wavepacket: StringWave):
    """TODO: Docstring for evaluate_displacement.
    ...
    """
    return wavepacket.get_a_0() * np.exp(
        -(position - wavepacket.get_phase_velocity() * time) / (wavepacket.std) ** 2
    )
